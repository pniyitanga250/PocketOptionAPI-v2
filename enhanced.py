...
def strategie():
    ...
    for pair in global_value.pairs:
        ...
        if 'history' in global_value.pairs[pair]:
            ...
            # Trading condition: Doji candlestick pattern
            if len(df) >= 1:
                ...
                # Define threshold for Doji: body size is very small compared to candle range
                doji_threshold = 0.1  # 10% of candle range

                is_doji = (body_size <= doji_threshold * candle_range)

                if is_doji:
                    # Determine action based on previous candle direction
                    if len(df) >= 2:
