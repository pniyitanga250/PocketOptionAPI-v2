...
def strategie():
    for pair in global_value.pairs:
        if 'history' in global_value.pairs[pair]:
            ...
            df['ma1'] = ta.SMA(df["close"], timeperiod=14)
            df['ma2'] = ta.SMA(df["close"], timeperiod=20)
            df['ma3'] = ta.SMA(df["close"], timeperiod=50)
            ...
            df.loc[(df['ma3'] < df['ma3'].shift(1)), 'ma3_trend'] = -1
            df['buy'] = 0
            df.loc[(qtpylib.crossed_above(df['ma1'], df['ma3'])) & (qtpylib.crossed_above(df['ma2'], df['ma3'])), 'buy'] = -1
            df.loc[(qtpylib.crossed_below(df['ma1'], df['ma3'])) & (qtpylib.crossed_below(df['ma2'], df['ma3'])), 'buy'] = 1
            df.loc[(df['rsi'] > 75) & (df['buy'] == -1), 'buy'] = -1
            df.loc[(df['rsi'] < 30) & (df['buy'] == 1), 'buy'] = 1
            df.loc[~(((df['rsi'] > 90) & (df['buy'] == -1)) | ((df['rsi'] < 15) & (df['buy'] == 1))), 'buy'] = 0
            if df.loc[len(df)-1]['buy'] != 0:
